...
import { Badge, Card, EmptyState, ErrorState, Loading, Money, Stat, StatusPill } from "@/components/ui";

export default function DashboardPage() {
  const { data, error, loading, refresh } = usePortal();

  if (loading) return <Loading label="Loading escrow position..." />;
    ...
    <div className="space-y-6">
      ...
      >
        <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div><dt className="text-xs uppercase text-slate-400">Depositor</dt><dd className="mt-0.5 text-sm font-semibold text-white">{e.depositor_name}</dd></div>
          <div><dt className="text-xs uppercase text-slate-400">Next of Kin</dt><dd className="mt-0.5 text-sm text-slate-200">{e.next_of_kin}</dd></div>
          <div className="sm:col-span-2"><dt className="text-xs uppercase text-slate-400">Residential Address</dt><dd className="mt-0.5 text-sm text-slate-200">{e.address}</dd></div>
          <div><dt className="text-xs uppercase text-slate-400">Custodial Status</dt><dd className="mt-0.5 text-sm text-slate-200">{e.custodial_status}</dd></div>
          <div><dt className="text-xs uppercase text-slate-400">Physical Asset</dt><dd className="mt-0.5 text-sm text-slate-200">{e.asset_description}</dd></div>
          <div><dt className="text-xs uppercase text-slate-400">Vault Reference</dt><dd className="mono mt-0.5 text-sm text-slate-200">{e.vault_reference}</dd></div>
          <div><dt className="text-xs uppercase text-slate-400">Certificate</dt><dd className="mono mt-0.5 text-sm text-slate-200">{data.certificate?.reference ?? "-"}</dd></div>
        </dl>
      </Card>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Stat label="Final Disbursement Amount" cents={b.final_disbursement_cents} tone="gold" hint="Total minus charges, held and released funds" />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card
          ...
          right={
            obligations.length > 0 && (
              <Link href="/obligations" className="btn-secondary !py-1.5 !text-xs">
                Manage
              </Link>
            )
          }
        >
          {obligations.length === 0 ? (
            <EmptyState title="No obligations recorded" hint="Verified tax obligations will appear here once posted." />
          ) : (
        ...
        <Card
          ...
          right={
            <Link href="/timeline" className="btn-secondary !py-1.5 !text-xs">
              Full timeline
            </Link>
          }
        >
            ...
            <ol className="space-y-2.5">
              {stages.slice(Math.max(0, (nextStage?.seq ?? 13) - 3), Math.min(stages.length, (nextStage?.seq ?? 13) + 1)).map((s) => (
                <li key={s.id} className="flex items-center gap-3">
                  <span
                    aria-hidden="true"
                    className={`h-2 w-2 shrink-0 rounded-full ${s.status === "COMPLETED" ? "bg-emerald-400" : s.status === "IN_PROGRESS" ? "animate-pulse bg-amber-400" : "bg-slate-600"}`}
                  />
      ...
      <Card title="Security Posture" subtitle="Controls enforced by this platform">
        <ul className="grid gap-2 text-[13px] text-slate-300 sm:grid-cols-2 lg:grid-cols-3">
          {[
